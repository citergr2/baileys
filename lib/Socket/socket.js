...
const makeSocket = (config) => {
    ...
    let handshakeReject;
    const handshakePromise = new Promise((resolve, reject) => { handshakeResolve = resolve; handshakeReject = reject; });
    ...
    const end = (error) => {
        ...
        if (!handshakeComplete && handshakeReject) {
            handshakeReject(error || new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed }));
        }
    };
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        if (!handshakeComplete) {
            try {
                ...
                throw new boom_1.Boom('Connection failed before pairing', { statusCode: Types_1.DisconnectReason.connectionClosed });
            }
        }
        if (pairKey) {
    ...
    ws.on('CB:failure', (node) => {
        ...
        if (reason === 401) {
            end(new boom_1.Boom('Connection Failure (Logged Out)', { statusCode: Types_1.DisconnectReason.loggedOut, data: node.attrs }));
        } else {
            end(new boom_1.Boom('Connection Failure', { statusCode: reason >= 500 ? Types_1.DisconnectReason.restartRequired : reason, data: node.attrs }));
        }
    });
    ...
    ws.on('CB:ib,,offline_preview', (node) => {
        ...
        }).catch(err => logger.warn({ trace: err.stack }, 'error in sending offline batch'));
    });
    ...
    // safety net: prevent unhandled promise rejections from killing the process
    process.removeAllListeners('unhandledRejection');
    process.on('unhandledRejection', (err) => {
        logger.error({ trace: err?.stack || err }, 'unhandled promise rejection caught');
    });
    return {
        type: 'md',
        ws,
